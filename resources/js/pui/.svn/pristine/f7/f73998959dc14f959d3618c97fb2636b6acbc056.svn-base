		...
		timer: function(fn, slice, times, context){
			...
			S.mix(context, {
				_timerId: window.setInterval(function(){
						if(times > 0 && index >= times - 1) S.clearTimer(context._timerId);
						//设置回调函数，用于调取用户设定轮询逻辑，参数为轮询次数，停止函数
						fn.call(context,index, (function(){return function(){S.clearTimer(this._timerId)}})());
						index++;
					},slice*1000),
				stop: function(){
				...
				restart: function(){
					this.stop();
					S.timer(fn, slice, times, context);
				}
			})
