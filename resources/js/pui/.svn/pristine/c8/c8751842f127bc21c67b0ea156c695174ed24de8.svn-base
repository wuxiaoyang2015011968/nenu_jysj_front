...
(function(S) {
	...
	S.app('PW');
    if(!window._pw_env) return;
    if(!PW.Env) PW.Env = {};
	//获取配置信息
	S.mix(PW.Env, _pw_env);
	
	//第三方类库包
	var tag = (PW.Env.tag)? PW.Evn.tag : 'stable';
    ...
    //如果pkgs存在，则自动配置包
	if(PW.Env.pkgs && S.isArray(PW.Env.pkgs)){
	    S.each(PW.Env.pkgs,function(o){
	        S.config({
                packages : [{
                    name : o.name, 
                    tag : tag,
    ...
    //             done;
    //         rd(function(){
    //             if(S.isArray(pl) && pl.length > 0){
    //                 S.use(pl.join(','), function(){
    //                     callback(KISSY)
    //                 })    
    //             }else{
    //                 callback(KISSY);
