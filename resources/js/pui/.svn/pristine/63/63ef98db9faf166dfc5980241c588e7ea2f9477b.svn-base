/*-----------------------------------------------------------------------------
* @Description: 日期格式化组件 (moment.js)
* @Version:     V1.0.1
* @author:      simon(406400939@qq.com)
* @date         2012.12.03
* ==NOTES:===============，主要功能是满足后台对于=============================
* v1.0.0(2012.12.03):
*   初始生成，主要功能是满足后台对于对于时间格式化的要求，主要格式为两种（后期可以添加）：
*       1.传入从自1970年到目标日期时间的毫秒数，自动解析为对应时间，获得相应方法
*       2.传入日期对象，自动解析为相应的时间格式
*   调用了第三方组件moment.js,官网地址：http://momentjs.com/
* v1.0.1(2012.12.18):
*   bug修复-传入字符串不解析的错误
* v2.0.0(2013/12/30 14:33:30):
*   将momentjs版本更新至2.5.0 ，并调试成功
* ---------------------------------------------------------------------------*/

KISSY.add('mod/moment',function(S){
    S.moment.months = ["一月", "二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"];
    S.moment.weekdays = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"];
    S.moment.weekdaysShort  = ["周日", "周一", "周二", "周三", "周四", "周五", "周六"];
    S.moment.meridiem = function (hour, minute, isLower) {
        if (hour < 9) {
            return "早上";
    ...
    PW.namespace('mod.Moment');
    PW.mod.Moment = function(o){
        try{
            if(S.isNumber(o)){
                return S.moment(o);
            }
            if(S.isString(o)){
                return S.moment(parseInt(o));
            }else{
                return S.moment(o.time);
            }
        }catch(err){
            S.log('日期格式化出错:');
            S.log(err);
...
//! license : MIT
//! momentjs.com
KISSY.add('moment/base',function(S){
