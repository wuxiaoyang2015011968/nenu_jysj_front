...
KISSY.add('mod/scroll', function(S){

    var
        MS_SCROLL = PW.Env.modSettings.scroll,
        config = {
            renderTo: 'html',
            zindex: 300,
            // cursor最小透明度
            ...
            // 双击放大
            dblclickzoom: false,
            // 当boxzoom 为true的时候，可使用手势放大
            gesturezoom:true,
            //  display "grab" icon for div with touchbehavior
            grabcursorenabled:true,
            // 滚动条隐藏方式 true=default / "cursor" = only cursor hidden / false = do not hide
            autohidemode: true,
            // 滚动条背景
            background:"",
            ...
            // cursor最小高度
            cursorminheight: 32,
            // can scroll native scrollable areas with mouse, bubbling mouse wheel event 
            preservenativescrolling: true,
            // 滚动条偏移  {left:0 ,top: 0},railoffset: {left: 9,top: 0},
            railoffset: false,
            // 允许向手机一样滚动跳动
            bouncescroll:true,
            // 空格键可以控制滚动
            ...
            //  for chrome browser, disable outline (orange hightlight) when selecting a div with nicescroll 
            disableoutline:true,
            // 允许水平滚动条
            ...
            // 横向滚动条方位, top|bottom
            railvalign:"bottom",
            //  nicescroll can use css translate to scroll content 
            enabletranslate3d:true,
            // nicescroll can manage mouse wheel events
            enablemousewheel:true,
            //  nicescroll can manage keyboard events 
            enablekeyboard:true,
            //  scroll with ease movement 
            smoothscroll:true,
            // click on rail make a scroll
            sensitiverail:true,
            // can use mouse caption lock API
            enablemouselockapi:true,
            // set fixed height for cursor in pixel
            ...
            scriptpath: PW.Env.puiWebsite + 'assets/scroll/img/'
        }

    PW.namespace('mod.Scroll');
    PW.mod.Scroll = {
        client: function(param){
            var opts = S.merge(config, MS_SCROLL, param);
            if( opts.renderTo && $(opts.renderTo).length > 0){
                return $(opts.renderTo).niceScroll(opts);
            }
            return null;
        }
    }
...
KISSY.add('scroll/core', function(S) {
