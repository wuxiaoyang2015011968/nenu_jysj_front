...
* 	初始生成 
* ---------------------------------------------------------------------------*/
KISSY.add('page/admin_page/business_handling/protocolDetail',function(S,detail,popPic,parameterHidden){
	PW.namespace('page.admin_page.business_handling.protocolDetail');
	PW.page.admin_page.business_handling.protocolDetail = function(){
		new detail();
		new popPic();
		new parameterHidden();
	}
},{
	requires:['protocolDetail/detail','protocolDetail/popPic','protocolDetail/parameterHidden']
})
/*---------------------------------审核，查看证明材料----------------------------------------*/
KISSY.add('protocolDetail/detail',function(S){
	var 
		$ = S.all ,
		on = S.Event.on ,
		Dialog = PW.mod.Dialog,
		that = this,
		protocolDetailIO = PW.io.admin_page.business_handling.protocolDetail,
		el = {
	...
	S.augment(detail , {
		...
		_addEventListener:function(){
				...
				$(".check-state input[value=3]").on("click",function(ev){
					...
					$(el.J_check_other_input).attr("disabled","disabled");	
            		$(el.J_check_other_input).hide();
					//不通过的话，协议编号部分为disabled
        			$(".new-protocol input[name=agreementNumber]").attr("disabled","disabled");
				});
				//新协议编号的验证部分，只能是数字且位数在1到20位之间，调用一个函数，
				// $('input[name=agreementNumber]' , el.J_new_protocol).on("change" , function(ev){
				// 	protocolVerify();
				// });
				...
				$('input[name=agreementNumber]' , el.J_new_protocol).on("change",function(ev){  
					...
					if(!(/^[0-9]{1,20}$/.test(new_protocol)) || (new_protocol == "") || (new_protocol_val_length > 20) ){
						...
						protocolDetailIO.protocolDetailProtocol(para,function(code,data,msg){
							if(code == 0){
								isProtocolFit = true;
								$(el.J_new_protocol_tip).text('');
							};
				...
				$(el.J_save_submit).on("click",function(ev){
					...
					if(isProtocolFit&&isChooseFit&&isOtherFit){
						...
						protocolDetailIO.protocolDetailCheck(para,function(code,data,msg){
							if(code == 0){
				...
				$('input' , el.J_edit_result).on("click",function(ev){
    				...
    				state_input[0].checked=true;
    					$(el.J_new_protocol).show();
        				$('input[type=text]' , el.J_new_protocol).removeAttr("disabled").val("");//清空文本框的值
        				$(el.J_new_protocol_tip).text('');
        				$(".yes").removeAttr("disabled");
						$(".not").attr("disabled","disabled");	
	...
	requires:['event','io/admin_page/business_handling/protocolDetail','mod/dialog']
})
/*---------------------------------------------------------------------------*/
/*放大图片部分*/
KISSY.add('protocolDetail/popPic' , function(S){
    var
...
/*---------------------------------------------------------------------------*/
/*传隐藏表单的值*/
KISSY.add('protocolDetail/parameterHidden' , function(S){
    var
