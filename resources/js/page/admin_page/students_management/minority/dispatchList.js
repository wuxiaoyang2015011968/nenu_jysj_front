...
KISSY.add('page/admin_page/students_management/minority/dispatchList', function(S, List) {
	PW.namespace('page.admin_page.students_management.minority.dispatchList');
    PW.page.admin_page.students_management.minority.dispatchList = function(param) {
        new List(param);
    };
},{
    requires:[
...
/*-------------------------------------------------------------------*/
/*开始改页面模块的js内容*/
KISSY.add('dispatchList/List', function(S) {
	var $ = S.all,
        /*on = S.Event.on, //绑定静态节点*/
        /*delegate = S.Event.delegate, //绑定静态节点*/

        minorityIO = PW.io.admin_page.students_management.minority.dispatchList, //定义一个IO层入口

        Pagination = PW.mod.Pagination,
        Defender = PW.mod.Defender,
        Dialog = PW.mod.Dialog,        //定义要使用的组件

        urls = PW.Env.url.admin_page.students_management.minority.dispatchList, //定义数据入口
                ...
                $('body').delegate('click',el.viewbtn,function(ev){
                    ...
                    minorityIO.nowInfo(para,function(code,data,msg){
                            if(code == 0){
                               ...
                               window.location.href="/admin/minority/dispatch/detail/" + id; 
                            }
                    });
                    //console.log(para);
                });
...
{
	requires:['event','sizzle', 'mod/pagination', 'io/admin_page/students_management/minority/dispatchList']
});