...
var ZYFILE = {
	...
	funUploadFile : function(file){
		...
		var id = $(".school-roll").attr("value");
		var stuId = $("input[name='stuId']").attr("value");
...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if(para.tailor){  // 显示裁剪按钮
					editHtml = '<span class="file_edit" data-index="'+file.index+'" title="放大编辑"></span>';
				}
				if(para.del){  // 显示删除按钮
			 ...
			 * 返回: 无
			 */
			this.createPopupPlug = function(imgSrc, index, name,id){
				// 初始化裁剪插件
				$("body").zyPopup({
					src        :   imgSrc,            // 图片的src路径
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									...
									return false;
									var name = $(this).parents("file_bar").siblings("imgBox").children("img").attr("alt");
						                $.ajax({
						                    url: "",
						                    data: "picname="+name,
						                    datatype: "json",
						                    ...
						                    success: function(data, textStatus) {
						                        $(this).parents("file_bar").siblings("imgBox").children("img").hide();
						                    },
